...
if(isset($_GET)) {
	$searchmobile = $_GET["searchmobile"];
}
...
if ($searchmobile <> "") {
	...
	$sql = "select * from registered_users where mobile ='".$searchmobile."'";
		if($res = $conn->query($sql)) {

			if($res->fetchColumn() > 0) {
				$error_message = "請更新以下資料";
				foreach ($conn->query($sql) as $row) {  

				$_SESSION["cName"] = $row['cName']; $_SESSION["eName"] = $row['eName'];
				$_SESSION["gender"] = $row['gender']; $_SESSION["hkid"] = $row['hkid'];
				$_SESSION["dob_year"] = $row['dob_year']; $_SESSION["dob_month"] = $row['dob_month'];
				$_SESSION["dob_date"] = $row['dob_date']; $_SESSION["height_cm"] = $row['height_cm'];
				$_SESSION["promotion"] = $row['promotion']; $_SESSION["hm_q1"] = $row['hm_q1']; $_SESSION["hm_q2"] = $row['hm_q2'];
				$_SESSION["hm_q3"] = $row['hm_q3']; $_SESSION["hm_q4"] = $row['hm_q4'];
				$_SESSION["hm_q5"] = $row['hm_q5']; $_SESSION["hm_q6"] = $row['hm_q6'];
				$_SESSION["hm_q7"] = $row['hm_q7']; $_SESSION["hm_q8"] = $row['hm_q8'];
				$_SESSION["hm_q9"] = $row['hm_q9']; $_SESSION["hm_q10"] = $row['hm_q10'];
				$_SESSION["hm_q_other"] = $row['hm_q_other'];
				$_SESSION["hm_team"] = $row['hm_team']; $_SESSION["hm_shop"] = $row['hm_shop'];
				$_SESSION["hm_buy"] = $row['hm_buy'];

				$_SESSION["hc_q1"] = $row['hc_q1']; $_SESSION["hc_q2"] = $row['hc_q2'];
				$_SESSION["hc_q3"] = $row['hc_q3']; $_SESSION["hc_q4"] = $row['hc_q4'];
				$_SESSION["hc_q5"] = $row['hc_q5']; $_SESSION["hc_q6"] = $row['hc_q6'];
				$_SESSION["hc_q7"] = $row['hc_q7']; $_SESSION["hc_q_other"] = $row['hc_q_other'];
				$_SESSION["hc_team"] = $row['hc_team']; $_SESSION["hc_shop"] = $row['hc_shop'];
				$_SESSION["hc_buy"] = $row['hc_buy'];

				$_SESSION["mobile"] = $row['mobile']; $_SESSION["userEmail"] = $row['userEmail']; 
...
if ($_POST["update"] == 1){
	...
	if ($_SESSION["cName"] <> $_POST["cName"]) { $_SESSION["cName"] = $_POST["cName"]; }
	if ($_SESSION["eName"] <> $_POST["eName"]) { $_SESSION["eName"] = $_POST["eName"]; }
	if ($_SESSION["gender"] <> $_POST["gender"]) { $_SESSION["gender"] = $_POST["gender"]; }
	if ($_SESSION["hkid"] <> $_POST["hkid"]) { $_SESSION["hkid"] = $_POST["hkid"]; }
	if ($_SESSION["dob_year"] <> $_POST["dob_year"]) { $_SESSION["dob_year"] = $_POST["dob_year"]; }
	if ($_SESSION["dob_month"] <> $_POST["dob_month"]) { $_SESSION["dob_month"] = $_POST["dob_month"]; }
	if ($_SESSION["dob_date"] <> $_POST["dob_date"]) { $_SESSION["dob_date"] = $_POST["dob_date"]; }
	if ($_SESSION["height_cm"] <> $_POST["height_cm"]) { $_SESSION["height_cm"] = $_POST["height_cm"]; }
	if ($_SESSION["promotion"] <> $_POST["promotion"]) { $_SESSION["promotion"] = $_POST["promotion"]; }
	
	if ($_SESSION["hm_q1"] <> $_POST["hm_q1"]) { $_SESSION["hm_q1"] = $_POST["hm_q1"]; }
	if ($_SESSION["hm_q2"] <> $_POST["hm_q2"]) { $_SESSION["hm_q2"] = $_POST["hm_q2"]; }
	if ($_SESSION["hm_q3"] <> $_POST["hm_q3"]) { $_SESSION["hm_q3"] = $_POST["hm_q3"]; }
	if ($_SESSION["hm_q4"] <> $_POST["hm_q4"]) { $_SESSION["hm_q4"] = $_POST["hm_q4"]; }
	if ($_SESSION["hm_q5"] <> $_POST["hm_q5"]) { $_SESSION["hm_q5"] = $_POST["hm_q5"]; }
	if ($_SESSION["hm_q6"] <> $_POST["hm_q6"]) { $_SESSION["hm_q6"] = $_POST["hm_q6"]; }
	if ($_SESSION["hm_q7"] <> $_POST["hm_q7"]) { $_SESSION["hm_q7"] = $_POST["hm_q7"]; }
	if ($_SESSION["hm_q8"] <> $_POST["hm_q8"]) { $_SESSION["hm_q8"] = $_POST["hm_q8"]; }
	if ($_SESSION["hm_q9"] <> $_POST["hm_q9"]) { $_SESSION["hm_q9"] = $_POST["hm_q9"]; }
	if ($_SESSION["hm_q10"] <> $_POST["hm_q10"]) { $_SESSION["hm_q10"] = $_POST["hm_q10"]; }
	if ($_SESSION["hm_q_other"] <> $_POST["hm_q_other"]) { $_SESSION["hm_q_other"] = $_POST["hm_q_other"]; }
	if ($_SESSION["hm_team"] <> $_POST["hm_team"]) { $_SESSION["hm_team"] = $_POST["hm_team"]; }
	if ($_SESSION["hm_shop"] <> $_POST["hm_shop"]) { $_SESSION["hm_shop"] = $_POST["hm_shop"]; }
	if ($_SESSION["hm_buy"] <> $_POST["hm_buy"]) { $_SESSION["hm_buy"] = $_POST["hm_buy"]; }

	if ($_SESSION["hc_q1"] <> $_POST["hc_q1"]) { $_SESSION["hc_q1"] = $_POST["hc_q1"]; }
	if ($_SESSION["hc_q2"] <> $_POST["hc_q2"]) { $_SESSION["hc_q2"] = $_POST["hc_q2"]; }
	if ($_SESSION["hc_q3"] <> $_POST["hc_q3"]) { $_SESSION["hc_q3"] = $_POST["hc_q3"]; }
	if ($_SESSION["hc_q4"] <> $_POST["hc_q4"]) { $_SESSION["hc_q4"] = $_POST["hc_q4"]; }
	if ($_SESSION["hc_q5"] <> $_POST["hc_q5"]) { $_SESSION["hc_q5"] = $_POST["hc_q5"]; }
	if ($_SESSION["hc_q6"] <> $_POST["hc_q6"]) { $_SESSION["hc_q6"] = $_POST["hc_q6"]; }
	if ($_SESSION["hc_q7"] <> $_POST["hc_q7"]) { $_SESSION["hc_q7"] = $_POST["hc_q7"]; }
	if ($_SESSION["hc_q_other"] <> $_POST["hc_q_other"]) { $_SESSION["hc_q_other"] = $_POST["hc_q_other"]; }
	if ($_SESSION["hc_team"] <> $_POST["hc_team"]) { $_SESSION["hc_team"] = $_POST["hc_team"]; }
	if ($_SESSION["hc_shop"] <> $_POST["hc_shop"]) { $_SESSION["hc_shop"] = $_POST["hc_shop"]; }
	if ($_SESSION["hc_buy"] <> $_POST["hc_buy"]) { $_SESSION["hc_buy"] = $_POST["hc_buy"]; }

	if ($_SESSION["mobile"] <> $_POST["mobile"]) { $_SESSION["mobile"] = $_POST["mobile"]; }
	if ($_SESSION["userEmail"] <> $_POST["userEmail"]) { $_SESSION["userEmail"] = $_POST["userEmail"]; }
	...
	$sql = "update registered_users 
			...
			height_cm = '". $_SESSION["height_cm"]."',
			promotion = '". $_SESSION["promotion"]."',

			hm_q1 = '". $_SESSION["hm_q1"]."', hm_q2 = '". $_SESSION["hm_q2"]."',
			hm_q3 = '". $_SESSION["hm_q3"]."', hm_q4 = '". $_SESSION["hm_q4"]."',
			hm_q5 = '". $_SESSION["hm_q5"]."', hm_q6 = '". $_SESSION["hm_q6"]."',
			hm_q7 = '". $_SESSION["hm_q7"]."', hm_q8 = '". $_SESSION["hm_q8"]."',
			hm_q9 = '". $_SESSION["hm_q9"]."', hm_q10 = '". $_SESSION["hm_q10"]."',
			hm_q_other = '". $_SESSION["hm_q_other"]."',
			hm_team = '". $_SESSION["hm_team"]."',
			hm_team_other = '". $_SESSION["hm_team_other"]."',
			hm_shop = '". $_SESSION["hm_shop"]."',
			hm_buy = '". $_SESSION["hm_buy"]."',

			hc_q1 = '". $_SESSION["hc_q1"]."', hc_q2 = '". $_SESSION["hc_q2"]."',
			hc_q3 = '". $_SESSION["hc_q3"]."', hc_q4 = '". $_SESSION["hc_q4"]."',
			hc_q5 = '". $_SESSION["hc_q5"]."', hc_q6 = '". $_SESSION["hc_q6"]."',
			hc_q7 = '". $_SESSION["hc_q7"]."', hc_q_other = '". $_SESSION["hc_q_other"]."',
			hc_team = '". $_SESSION["hc_team"]."',
			hc_team_other = '". $_SESSION["hc_team_other"]."',
			hc_shop = '". $_SESSION["hc_shop"]."',
			hc_buy = '". $_SESSION["hc_buy"]."',

			mobile = '". $_SESSION["mobile"]."',
			userEmail = '". $_SESSION["userEmail"]."',
			updatetime = '". $_POST["updatetime"]." '

			where mobile ='".$searchmobile."'";
	    	...
	    	// set the PDO error mode to exception
			$result = $db_handle->updateQuery($sql);
					
					//	if(!empty($result)) {
						if ($result == "1"){
							$error_message = "";
							$success_message = "已成功更新";
...
?>
<option  selected><?php echo $_SESSION['dob_year']?></option>
</select>年
...
?>
<option  selected><?php echo $_SESSION['dob_month']?></option>
</select>月
...
?>
<option  selected><?php echo $_SESSION['dob_date']?></option>
</select>日
	</td>
</tr>
<tr>
...
<BR>
	...
	<span>選購優惠 :
		<label>		
		    <input type="radio" name="hm_buy" value="$698-三項健康" <?php if(isset($_SESSION['hm_buy']) && $_SESSION['hm_buy']=="$698-三項健康") { ?>checked<?php  } ?>>
	...
	<div id="tabs-2">
		...
		<BR>
		    ...
		    <label>
			    <input type="radio" name="hc_buy" value="$688/3次-純中藥外敷療程" <?php if(isset($_SESSION['hc_buy']) && $_SESSION['hc_buy']=="$688/3次-純中藥外敷療程") { ?>checked<?php  } ?>>
			    $688/3次-純中藥外敷療程
		    </label>
		    <label>		
			    <input type="radio" name="hc_buy" value="$788/3次-中藥頭風療程" <?php if(isset($_SESSION['hc_buy']) && $_SESSION['hc_buy']=="$788/3次-中藥頭風療程") { ?>checked<?php  } ?>>
			    $788/3次-中藥頭風療程
		    </label>
		     <label>		
...
<input type="submit" name="updateuser" value="更新資料" class="btnRegister">
